# CLAUDE.md — SlideAtlas 프로젝트 메모리 v3.13
...
- **과목축 분리→기관 롤업(§18 D9)**: active_users·max_seats·소진율을 (기관×과목)으로 산출, 'all'은 과목별 합(SUM, 단일 사용자=단일 subject_code라 중복 없음). 구성원 활동은 status 기반(활성=active/미인증=pending/비활성=그외).
...
- 내부 QA(a)스코프격리 (b)과목격리(비구독 403) (c)수식주입 방어 (d)집계 과목별 산출→롤업 (+)빈데이터 graceful — pytest 183(P3 15 추가). **포털 3탭(P1·P2·P3) 완성.** P2+P3 묶은 외부검증(Codex/Gemini) 1회는 다음 단계 판단.
