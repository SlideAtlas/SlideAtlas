# CLAUDE.md — SlideAtlas 프로젝트 메모리 v3.0

> 이 파일은 Claude Code 세션 시작 시 반드시 읽어야 하는 프로젝트 컨텍스트 파일입니다.
> 모든 에이전트(오케스트레이터, 개발, QA)는 이 파일을 기준으로 작업합니다.
>
> **v3.0 핵심 변경 — 과목/좌석 분리 정책을 문서 전체에 일관 반영.**
> 데이터 모델(§7·§16)은 v2.9부터 (기관×과목) 단위였으나, 서술·코드 일부가 옛 "기관 단위" 모델을
> 따라가지 못해 혼선이 있었다. v3.0은 **구독·좌석·만료·접근·집계가 모두 (기관 × 과목) 단위로 독립**
> 한다는 원칙을 §6·§7·§8·§9·§15·§16 전반에 정렬했고, v1.0(HST 단일)이 그 구조의 *특수 케이스일 뿐*
> 임을 명문화했다. 의도적으로 미룬 항목은 **§18 기술부채**에 집결한다.
...
- ⚠ **회귀 주의(QA 발견)**: 과거 코드가 가입 시 subject_code를 채우지 않아 전 사용자 NULL이었다.
  ...
  NULL 사용자에 대한 폴백(기관 단위 처리)은 **과도기 안전장치일 뿐 정상 경로가 아니다**(§18).
...
- **타일 접근 토큰**: TTL 5분, HMAC-SHA256. 뷰어 로드 시 `generate_tile_token(user_id, institution_id, slide_id)` 발급 → 모든 타일/DZI URL에 `?t=`. `verify_tile_token`은 user_id·institution_id·slide_id·exp **세 요소 모두 대조**(QA 확인됨). 검증 실패 401. S3 퍼블릭 차단.
...
- **멀티테넌시**: institution_id 기반 Row Level 격리. 모든 슬라이드/타일/DZI 라우트에 `@login_required`(또는 `@page_login_required`) + institution_id 검사.
- **과목 격리 (v3.0 명문화)**: 콘텐츠 접근은 (institution_id, subject_code) 양축으로 격리한다. 즉 사용자는 자신의 기관 + 자신이 구독/등록한 과목의 슬라이드만 접근. v1.0은 HST 단일이라 결과적으로 기관 격리와 같아 보이나, **코드는 subject_code 축을 반드시 검사**해야 한다(§0-4). ⚠ 과거 코드가 과목 축 미검사 → §18 부채.
...
- **구독 만료 매 요청 검사 (v3.0 정정)**: `_authenticate()`에서 매 요청, 해당 user의 **(institution_id, subject_code)에 매칭되는 `subscriptions.subscription_end`**를 확인한다. 옛 `institutions.subscription_end`(deprecated) 참조 금지. 매칭 구독이 없거나 만료면 SUBSCRIPTION_EXPIRED(401). is_special 예외.
- **401 코드 구분**: SESSION_REVOKED(타 기기 로그인) / TOKEN_INVALID(세션 없음) / TILE_TOKEN_INVALID(타일토큰 검증실패, 로그인과 무관 — **리다이렉트 금지, 뷰어 JS 재발급/토스트로 분기**) / SUBSCRIPTION_EXPIRED. interceptor.js가 4종을 구분 처리.
...
1. **보안·멀티테넌시**: 타 기관 슬라이드 URL 조작 차단, **과목 축(subject_code) 격리**, JWT 변조 방어, 1기기 동시접속, 타일토큰 3요소 대조·TTL, no-store 헤더, 계정잠금(24h/10회), 인증코드 재발송 한도, CSRF(interceptor 전 fetch 경로 적용), **과목별 구독 만료 매요청 검사**, /api/chat 탈옥 방어. **어드민 권한 게이트(super_admin/staff) 우회 불가.**
2. **파이프라인 안전성**: COG 스트리밍(전체 메모리 로드 금지), QC 실패·ready_no_mpp가 ready로 전환 안 됨, 미니맵·썸네일 S3 경로 정확, ready_no_mpp 배율 버튼 비활성.
3. **비즈니스 로직**: 만료 사용자 접근 차단+결제 유도, **변환 ready여도 deploy_status='deployed' 아니면 학생 비노출**, **과목별 좌석 카운터(max_seats) 정확**.
...
- 답변은 SES 발송, 작성자 기록(created_by). 상태 open/answered.
...
- **접근 오픈일 = 학기 시작 −30일**(봄 2/1, 가을 8/1). 학기 첫날 혼란 방지 위해 한 달 일찍 오픈(편의 제공이며 라이선스 기간과 별개). 신규·재구독은 이전 만료일이 없으므로 겹침 문제 없음.
  - ⚠ 날짜 계산은 타임존(KST) 기준으로 일관 처리. access_open_date/subscription_end 자동계산이 UTC와 하루 어긋나지 않도록 주의. (QA 잠재 위험 지적.)
...
| D3 | ADMIN_SECRET_KEY 환경변수 | Render 환경변수 설정(완료 확인됨, 재확인) | 🔴 출시 전 필수 | 확인 |
| D4 | users.subject_code 채번 | ✅ 완료(코드, commit 17bb18a). register()가 roster의 subject_code 캡처→users 채번, verify_email() 누락 거부, login·_authenticate NULL 폴백 제거. 정원도 subscriptions.max_seats 이전(ddfab51). v1.0 사용자 0이라 백필 불필요(라이브 DB count는 CEO/EC2 영역). pytest 45/45 | ✅ 완료 | Lead Developer |
...
| D9 | CSV 수식 주입 방어 | 리포트 엑셀 내보내기 셀 값 이스케이프(=,+,-,@) | 🟡 추적 | Lead Developer |
| D10 | 날짜 타임존 일관성 | access_open_date/subscription_end KST 기준 일관 처리 | 🟡 추적 | Lead Developer |
...
*최종 업데이트: 2026-05-31 v3.0 | 변경: §0 단일 진실 원칙 신설(구독=기관×과목, institutions 옛 컬럼 deprecated, users.subject_code 가입 시 필수 채번, HST 단일은 우연) / §6-2 사용자–과목 매칭 명문화 / §7 institutions 옛 컬럼 DEPRECATED 표기·inquiries 스키마 정정 / §8 과목 격리·만료 검사 정정·interceptor CSRF·401 4종 / §9 포털 과목별 좌석 현황 / §11 콘텐츠 조달 방식 확정·공급사 율린vsVic 미정(Vic 견적 대기) / §13-2 인증·구독 불변식 신설 / §15 전반 과목 단위 정렬·is_special rejected 차단 / §16 정원검사 max_seats / §18 기술부채 단일 집결지 신설 | 직전: v2.9 | 다음: D4(subject_code 채번) 코드 수정 → Codex 외부 검증*
