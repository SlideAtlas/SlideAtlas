...
| 뷰어 | OpenSeadragon |
| 배포 | Render Starter ($7/월) (개발 단계: Render. 베타 전 EC2 이전 예정 — §18 D16) |
...
| D16 | Render→EC2 앱 이전 (정식 인프라) | 현재 Flask 앱이 외부 PaaS(Render)에서 구동돼 VPC 내부 RDS에 외부 접속하는 임시 구조. 정석은 RDS와 같은 VPC의 EC2(기존 t3.medium 타일서버)에서 Flask를 gunicorn+nginx+TLS로 구동해 RDS를 완전 프라이빗으로 유지하는 것. **모든 포털·기능 개발 완료 후 클로즈드베타(7월) 직전 EC2로 이전**하고 RDS 퍼블릭 액세스를 다시 닫는다. | 🔴 출시 전 필수(베타 전) | CEO |
| D17 | RDS 임시 퍼블릭 접속 회수 | 개발·내부 e2e 위해 RDS를 퍼블릭 액세스로 두고 Render 아웃바운드 IP만 보안그룹 화이트리스트한 임시 상태. D16(EC2 이전) 완료 시 퍼블릭 액세스 '아니요'로 되돌리고 Render IP 인바운드 규칙 제거, EC2 보안그룹만 유지. | 🔴 출시 전 필수(베타 전) | CEO |
...
> ⚠ 현재 RDS는 개발용으로 퍼블릭 액세스 임시 허용(Render IP 화이트리스트). 베타 전 EC2 이전 후 프라이빗 전용으로 회수 — §18 D16·D17.
