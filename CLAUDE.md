# CLAUDE.md — SlideAtlas 프로젝트 메모리 v3.2

> 이 파일은 Claude Code 세션 시작 시 반드시 읽어야 하는 프로젝트 컨텍스트 파일입니다.
> 모든 에이전트(오케스트레이터, 개발, QA)는 이 파일을 기준으로 작업합니다.
>
> **v3.2 핵심 변경 — 정책 다수 확정(운영·집계·데이터 보존).**
> ① **기관 관리자 포털(/portal)을 v1.0 출시 필수로 승격**(scope 격리·명단 xlsx·개별 추가/삭제·삭제 시 좌석 반환·포털 리포트). 현재 구현 0건(§9·§18 D15). ② **집계 2축 명문화**: 현재값=접근창 내 active 구독 기준·과목별→롤업·만료 시 0 동결(`users.status` 의존 금지), 이력=선택 기간 로그+구독 기준(§15-7·§16-1). ③ **§20 데이터 보존·익명화·계정 생애주기 신설**: 로그 5년 보존, 만료 1년 경과 시 휴면+익명화(비가역), 약관 동의 DB 저장. ④ 다과목 명단은 v1.5(스키마 마이그레이션 D12).
>
> **v3.1 골격(유지) — 과목 구독 단일 게이트·구독 선행 온보딩·접근창(KST) fail-closed.**
> 슬라이드 접근은 **단일 게이트(`_slide_access_allowed`)**(§8), 온보딩은 **구독 선행**(§6-3), 만료·가입은 **접근창(`access_open_date<=today<=subscription_end`, KST)·fail-closed**(§8·§16).
>
...
> ★ **상태(2026-06): 설계는 있으나 구현 0건.** `institution_rosters`는 현재 코드에서 읽기(좌석 카운트) 1곳만 참조하고, 학생 추가/삭제·명단 업로드·포털 리포트 라우트가 **전무**하다(§18 D9·D15). 아래 항목은 **전부 v1.0 출시 필수(🔴)**로 못박는다 — 명단 운영 없이는 온보딩(§6-3)이 성립하지 않기 때문이다.

- **로그인·scope 격리(멀티테넌시 1급 규칙, §8)**: 기관 관리자 로그인 → `/portal`. scope는 **자기 기관(`institution_id`)으로 강제** — 다른 기관 데이터는 어떤 경로로도 접근 불가. **super_admin은 기관을 선택해 동일 작업을 대행**(전체 기관). 이 scope 격리는 슬라이드 접근 단일 게이트와 동급의 1급 멀티테넌시 규칙으로 다룬다(§8).
- **학생 명단 관리 (v1.0 단일 과목 전제, §2[다과목]·§18 D12)**:
  - xlsx 업로드(이름 + 이메일). **v1.0은 단일 과목 전제**이므로 업로드 시 해당 과목 1개에 귀속된다(다과목 명단은 v1.5 — 아래).
  - 개별 추가/삭제, **인라인 편집**.
  - **명단 교체 vs 병합 옵션**: 업로드 시 "기존 명단 전체 교체" / "기존에 추가 병합" 선택.
- **삭제 시 즉시 접근 차단 + 좌석 반환**: roster 행 삭제 → 해당 사용자 즉시 접근 불가 + 과목 좌석 1 반환. (영구 이탈 처리의 익명화·유예는 §20 계정 생애주기.)
- **좌석 현황**: **"사용 중 N / 전체 N"**(과목당 사용 N / max_seats N). 집계 기준은 §16 집계 원칙(a)를 따른다 — 접근창 내 active 구독 보유 사용자만 카운트.
- **포털 내 이용 리포트**: 과목별 활성·좌석 소진율, 총 열람, AI 튜터 질문수, 많이 본 슬라이드 Top N, 로그인 추세, 마지막 활동. **집계 기준은 §16 집계 원칙·§15-7을 따른다**(현재값=접근창 내 active 구독 기준, 이력=선택 기간 로그·구독 기준). 기관 관리자는 자기 기관만, super_admin은 학교 선택해 동일 열람(§15-7).

> **다과목 명단은 v1.5 확장**: 한 학생이 여러 과목 명단에 동시에 들어가려면 `users.email UNIQUE` / `institution_rosters UNIQUE(institution_id, email)`를 `(institution_id, subject_code, email)`로 **마이그레이션**해야 한다(§18 D12). v1.0 단일 과목 포털에서는 불필요하며, v1.5 다과목 출시 시 선행한다.
...
구독 기관이 늘어 관리 인력을 붙일 경우, 현재 2단계(super_admin/staff)에 중간 역할 'manager'를 추가하는 것을 염두에 둔다. manager는 기관 관리자 가감, **명단 관리(기관 단위)**, 슬라이드 파이프라인 감독, 이용 리포트(건수) 등 운영 업무를 위임받되, **재무 데이터(매출·fee·매출 추이)는 어떤 역할 확장에서도 super_admin 전용**으로 유지한다. 즉 재무 격리선은 역할이 몇 개로 늘든 1급 권한 게이트 기준으로 불변이다. v1.0은 현재 2단계 권한을 그대로 유지하며, manager 역할·ops 대시보드는 실제 인력 투입 시점에 별도 설계·구현한다.
...
- **집계 2축(§16 집계 원칙)**:
  - **(현재값)** 좌석·활성 = **접근창 내 active 구독 보유 사용자만** 카운트, 과목별 산출→기관 롤업. 구독 만료 시 해당 과목 활성/좌석 **0으로 동결**. `users.status` 의존 금지(§0 단일 진실). *현재 코드가 `users.status`·roster 카운트로 제각각인 것은 결함(§18 D9)이며 [집계 수정 작업]에서 처리.*
  - **(이력)** 학기·연도 드롭다운으로 **과거 기간 조회**. 이력 집계는 현재 상태가 아니라 **선택 기간의 `access_logs`·chat 로그 타임스탬프 + 해당 기간 `subscriptions` 레코드** 기준. **만료된 과거 학기도 조회 가능.** 기관 관리자는 자기 기관 이력만, super_admin은 기관 선택+전체.
  - **(KST)** 리포트·대시보드 기간 계산의 `date.today()`(서버시각) 잔여 경로는 `_today_kst`로 통일 필요(§18 D10 residual).
...
### 16-1. 집계 원칙 (현재값 vs 이력 — §0·§9·§15-7 연계)
집계는 "지금 이 순간의 현황"과 "과거 기간 이력"을 **다른 기준**으로 산출한다. 혼선 방지를 위해 상위 규칙으로 둔다.

- **(a) 현재값 (좌석·활성)**: **접근창 내 active subscription(`access_open_date <= today <= subscription_end`, KST) 보유 사용자만** 카운트한다. **과목별 산출 → 기관 롤업.** **구독 만료 시 해당 과목 활성/좌석은 0으로 동결**된다. **`users.status`에 의존 금지**(§0 단일 진실) — 만료돼도 `users.status='active'`로 남으므로 status 기준은 만료를 반영하지 못한다. ⚠ 현재 코드가 `users.status`(대시보드 KPI·리포트 KPI)·roster 카운트(기관목록 used_seats)로 **제각각인 것은 결함**(§18 D9)이며 [집계 수정 작업]에서 subscriptions 기준으로 통일 예정.
- **(b) 이력 (이용 리포트 과거 조회)**: 학기(드롭다운)·연도(드롭다운)로 과거 기간을 조회한다. 이력 집계는 **현재 상태가 아니라 선택 기간의 `access_logs`·chat 로그 타임스탬프 + 해당 기간 `subscriptions` 레코드** 기준이다. **만료된 과거 학기도 조회 가능.** 기관 관리자는 자기 기관 이력만, super_admin은 기관 선택+전체.
- **(c) KST 일관성**: 리포트·대시보드 기간 계산이 `date.today()`(서버시각, UTC)를 쓰는 잔여 경로가 있다 → `_today_kst`로 통일 필요(§18 D10 residual).
...
| ID | 항목 | 내용 | 상태 | 조치 주체 |
|----|------|------|------|-----------|
| D1 | inquiries.privacy_agreed 컬럼 + 약관 동의 저장 | 개인정보 동의 저장 공백(법적). 문의 폼은 동의받으나 DB 미저장. **§20 데이터 보존·익명화 약관 동의 저장과 묶어 출시 전 필수**(약관 동의도 DB 저장) | 🔴 출시 전 필수 | CEO 승인 → ALTER |
...
| D9 | 리포트 집계 = 접근창 내 active 구독 기준 통일 (Codex #7) | 집계가 과목별 산출→기관 롤업이어야 한다(§9·§15-7·§16-1). **현재 코드는 기준이 제각각**: 대시보드 KPI(`server_render.py` 활성사용자=`COUNT(users WHERE status='active')`, subscriptions 미조인)·리포트 KPI(동일 status 기준 + `subject_code` 필터 없음, 기관 전체 합산)·기관목록 used_seats(`COUNT(institution_rosters)` roster 행 수). 만료 시 `users.status`를 바꾸는 경로 부재 → 만료돼도 활성/좌석 0 미동결. → subscriptions(접근창 내 active) 기준·과목별로 통일 필요(§16-1 a). | 🟠 v1.5 전(집계 정확성) | Lead Developer |
| D10 | 날짜 타임존 일관성 | ✅ 인증·접근창·만료·가입 경로는 `_today_kst`(KST)로 통일 완료(v3.1). **잔여(residual): `server_render.py` 리포트(`_reports_date_range`)·대시보드 KPI가 `date.today()`(서버시각/UTC) 사용 → `_today_kst`로 통일 필요**(§15-7 c·§16-1 c) | 🟡 추적(인증 경로 완료, 리포트/대시보드 잔여) | Lead Developer |
| D11 | DB 커넥션 release 전수 | get_db_conn/release_db_conn 누수 전수 카운트 미완 | 🟡 추적 | QA |
| D12 | 다중 과목 N-레코드 인증 (Codex 2-2#1) | 한 이메일이 여러 과목 명단/구독 시 과목별 레코드 분리. **CLAUDE.md §7은 UNIQUE(institution_id, subject_code, email)로 명시하나, 실제 DDL은 미반영**: `db/schema.sql` `users.email VARCHAR(200) UNIQUE`(전역) + `db/auth_schema.sql` `institution_rosters UNIQUE(institution_id, email)`(subject_code 없음). register도 `ORDER BY subject_code LIMIT 1`로 단일 과목만 채번. → v1.5 다과목 출시 전 **UNIQUE 제약 마이그레이션 필수**. v1.0 단일 과목은 안전 | 🟠 v1.5 전 | Lead Developer |
...
| D15 | 기관 관리자 포털(/portal) 구현 | **설계만 있고 구현 0건**(§9). `institution_rosters`는 읽기 1곳만 참조, 학생 추가/삭제·xlsx 업로드·삭제 시 좌석 반환·포털 리포트·scope 격리 라우트 전무. 온보딩(§6-3)·명단 운영의 전제라 **v1.0 출시 필수** | 🔴 출시 전 필수 | Lead Developer |
| D16 | 데이터 보존·익명화·계정 생애주기 구현 (§20) | 로그 5년 보존·파기, 만료 1년 경과 휴면(dormant)+익명화, 개별 삭제 6개월 유예 후 익명화, 약관 동의 DB 저장(D1 연계). 배치/스케줄러·익명화 처리·휴면 안내 UX 미구현 | 🔴 출시 전 필수(정책)·🟠 배치 v1.5 | CEO/Lead Developer |
...
## 20. 데이터 보존·익명화·계정 생애주기 (v3.2 신설 — CEO 확정 정책)

> 개인정보·이용 로그의 보존·파기·익명화 정책과 계정 상태 생애주기를 한곳에 못박는다. 이용약관·포털 표시·집계 기준의 근거가 된다. **익명화는 비가역**임을 전제로 설계한다.

### 20-1. 이용 로그 보존·파기
- **이용 로그(`access_logs`·chat 로그) 5년 보존**, 5년 경과 시 해당 로그 **파기**.
- 보존 기간 내 로그는 이력 리포트(§16-1 b)의 근거 데이터다.

### 20-2. 익명화 정의 (방식 A — 행 보존 + PII 파기)
- **익명화 = `users` 행 삭제가 아니라 이름·이메일을 복구 불가하게 파기**하는 것(방식 A). `access_logs` 외래키와 익명 통계는 **보존**한다.
- 익명화 후 통계는 **"익명 인원수·열람 건수"로만** 조회되며 **개인 역추적 불가**. 이는 §15-7 "개별 학생 추적 지양"과 일치한다.
- **비가역 → 복구 시스템 없음.** 재등장은 재구독 + 새 명단 재등록으로만(§6-3 온보딩 재사용). **기관 레코드·구독 이력(`subscriptions`/`subscription_history`)은 익명화 대상이 아니다.**

### 20-3. 계정 생애주기
| 단계 | status | PII·이력 | 접근 | 비고 |
|------|--------|----------|------|------|
| 재학·구독중 | active | 실명 보존 | 가능 | 정상 |
| 구독 만료(명단 유지) | **active 유지** | 실명·이력 보존 | 접근 가능 과목 0 | **구독 종료는 익명화 트리거가 아니다.** 재구독 시 즉시 복원 |
| 만료 1년 경과 | **dormant(휴면)** | **즉시 익명화** | 차단 | 로그인 시도 시 아래 안내 문구 |

- **만료 1년 경과 → 휴면(dormant) 전환 + 즉시 익명화.** 로그인 시도 시 안내:
  > "소속 학교의 구독이 종료되어 휴면 처리되었습니다. 다시 이용하려면 학교의 재구독 후 명단 재등록이 필요합니다. 학교 측에 문의해 주세요."
- **휴면 트리거**: v1.0은 **A(기관 구독 1년 미갱신)만**. B(개인 장기 미접속)는 향후 확장.
- **휴면은 로그인 안내용 UX 상태일 뿐 집계 기준이 아니다** — 집계는 끝까지 `subscriptions`(접근창 내 active) 기준(§16-1 a). dormant라는 status 값으로 집계하지 않는다.

### 20-4. 명단 개별 삭제 (영구 이탈 — 별도 트랙)
- 포털에서 학생을 개별 삭제(영구 이탈)하는 경우는 휴면과 별도 트랙: **6개월 유예 후 익명화**(실수 복구창). 유예 기간 내 재등록 시 복원, 경과 시 §20-2 익명화.
- (삭제 즉시 접근 차단·좌석 반환은 §9 — 이는 익명화와 별개의 즉시 효과.)

### 20-5. 약관·동의 저장
- 위 보존·익명화 정책을 **이용약관에 명기**하고, **기관 관리자·super_admin 포털에 표시**한다.
- **약관 동의는 DB 저장**한다(§18 D1 `inquiries.privacy_agreed` 공백과 함께 처리 — 출시 전 필수).

---

*최종 업데이트: 2026-06-01 v3.2 | 변경(정책 다수 확정): §9 기관 관리자 포털 v1.0 출시 필수 승격·범위 확정(scope 격리·xlsx·개별 추가/삭제·교체vs병합·좌석 반환·포털 리포트, 구현 0건 명시) / §15-2 manager 노트에 '명단 관리(기관 단위)' 추가(재무 super_admin 전용 불변) / §15-7·§16-1 집계 2축 명문화(현재값=접근창 내 active 구독 기준·과목별→롤업·만료 시 0 동결·users.status 의존 금지 / 이력=선택 기간 로그+구독 기준 / KST residual) / §20 신설(로그 5년 보존·익명화 방식A·계정 생애주기 active→dormant+익명화·6개월 유예·약관 DB 저장) / §18 D1·D9·D10·D12 코드근거 보강·D15(포털)·D16(보존·익명화) 신설 | 직전: v3.1(2026-06-01 외부검증 반영) | 이전: v3.0 | 다음: D9·D12(v1.5) / D1·D2·D4b·D13·D15·D16 출시 전 / D14 부하테스트(7월 말)*

<!-- v3.1 변경 이력(보존) -->
*v3.1 변경(Codex 외부 검증 묶음 A·B 반영, pytest 65/65): §6-1 'SA=소유자 표시일 뿐 공용/기본제공 아님'·HST는 첫 런칭 과목일 뿐 기본제공 아님 명문화 / §6-3 온보딩 순서 원칙 신설(구독 선행, 없으면 SUBSCRIPTION_INACTIVE) / §8 슬라이드 접근 단일 게이트(`_slide_access_allowed`, 기관일치 화석 제거)·접근창 집행·특별계정 만료·ADMIN_SECRET_KEY fail-closed·타일토큰 재발급·문의답변/XLSX/XSS 방어 / §12 5대 체크리스트 단일 게이트·온보딩 반영 / §15-10 answered 발송성공 시에만 / §16 KST 접근창·베타 6개월무료→전환 모델 일반화·HST 자동부여 표현 정정 / §18 D3·D10 완료, D9(Codex #7)·D12(2-2#1) 유지, D13 온보딩 체크리스트·D14 Locust(7월말) 신설, v3.1 종결항목 요약 | 2026-06-01: 외부검증 반영 — §15-2 대시보드 staff ❌(재무 데이터 포함 super_admin 전용, `/admin/api/dashboard`=@super_admin_required)·향후 manager 역할 확장 노트(재무 격리선 불변)·§15-11 재무 KPI super_admin 전용 명시 | 직전: v3.0 | 다음: D9·D12(v1.5) / D1·D2·D4b·D13 출시 전 / D14 부하테스트(7월 말)*
