# CLAUDE.md — SlideAtlas 프로젝트 메모리 v3.19
...
-- organs (v1.0 신설, §18 D28): organ_code VARCHAR(20) PK, name_ko, name_en, system VARCHAR(40)(계통: 소화기/순환기/호흡기 등), display_order INT, is_active BOOLEAN DEFAULT TRUE, created_at
...
| D28 | organ 통제어휘 정규화 (v1.0) | normal-abnormal 연동 앵커. `organs` 참조 테이블 + `slides.organ_code`, 138종 적재 시 정규 코드 부여. 자유텍스트 organ 잔존은 v1.5 정리. | 🔴 출시 전(연동 선설계) | Lead Developer |
