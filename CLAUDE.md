...
- 전체 사용자 조회, 권한 부여/회수
- 슬라이드 관리 (현재 구현)
- 전체 현황 대시보드 (기관수·사용자수·구독현황·슬라이드 통계)

### 기관 관리자 (/portal)
- 계약 체결 후 슈퍼관리자가 기관 관리자 이메일 등록
- 해당 이메일 로그인 시 /portal 자동 접근
- 학생 명단: xlsx/csv 업로드(이름+이메일) → DB 등록
- 개별 학생 추가(이름+이메일 입력) / 삭제
- 학생 가입 시 명단 이메일 대조 인증

### 라이선스 모델
- 구독 = 기관당 활성 계정 수 라이선스 (예: 연세대 50석)
- 명단 삭제 → 즉시 접근 차단 + 라이선스 1 반환
- 신입생 추가 → 라이선스 1 소비
- 포털 대시보드에 "사용 중 N / 전체 N" 현황 표시
- 삭제 시 확인 팝업: "OOO을 삭제하면 즉시 접근이 차단됩니다" 경고
- 이미 가입한 학생 삭제 시 세션 강제 종료 여부 → 추후 결정

### 엣지케이스
- 명단 업로드 방식: 교체 vs 병합 선택 옵션 필요
- 동일 이메일이 여러 기관 명단에 존재할 경우 처리 방식 미정
---
*최종 업데이트: 2026-05-17 | 다음 업데이트: Happy Science 계약 완료 시점*
