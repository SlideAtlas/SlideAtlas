...
| D12 | 다중 과목 N-레코드 가입 | 한 이메일이 여러 과목 구독 시 과목별 레코드 분리. 구조는 마련됨(UNIQUE(institution_id, subject_code, email)), 단 register/verify/login이 이메일 키 단일 레코드 모델이라 완전 구현은 미완. v1.5 다과목 시 필요 | 🟠 v1.5 전 | Lead Developer |
