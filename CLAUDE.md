# CLAUDE.md — SlideAtlas 프로젝트 메모리 v3.25
...
| 타일엔진 | ⚠ **목표≠현행**: 목표=libvips 사전 COG 변환→S3→타일서버 range 읽기. **현행 코드는 openslide DeepZoom 동적 타일링**(~/tileserver/main.py). 기존 "titiler+rasterio" 서술은 코드와 불일치였음(2026-06-12 정찰 확인). openslide→COG 타일서버 전환은 별도 미래작업=부하방어 실제 해결지점(§18 신규 D36). |
| 변환 파이프라인 | SVS/DCM/TIFF/NDPI/VSI/**BIF**(뷰웍스 Roche BIF BigTIFF, v1.0 실입력 §11) → **libvips(pyvips)** COG TIFF → S3 → 타일서버 (변환 라이브러리 libvips 확정 §4) |
...
### 4-1. 설계 원칙
변환 스펙을 고정 — 어떤 입력이든 출력은 동일한 표준 COG TIFF. **변환 라이브러리 = libvips(pyvips) 확정**(2026-06-12 SVS 실측). 검증된 명령:
```
vips tiffsave IN OUT.tif --tile --tile-width 256 --tile-height 256
     --pyramid --compression jpeg --Q 85 --bigtiff
```
> ★ **libvips 채택 실측 근거**: 같은 Motic SVS에서 libvips=9레벨 단조감소 완벽 피라미드(26초), GDAL=3레벨 부실 피라미드(37초). 입력 피라미드가 깨져 있어도(Motic 레벨순서 엉킴·openslide level_count=1) libvips 가 표준 피라미드로 복구함(확인).

**고정 변환 스펙 (모든 슬라이드 공통)**
```
타일: 256×256 px / 압축: JPEG Q=85
오버뷰: libvips --pyramid 가변 레벨 (이미지 크기에 따라 결정; 41675px 입력에서 9레벨 관측)
        ⚠ 기존 '7레벨 고정(2..128)' 전제는 실측으로 깨짐 — libvips 는 타일 크기까지 반복 1/2 축소.
        실제 생성 레벨 수를 slides.overview_levels(전용 컬럼)에 기록.
MPP: 원본에서 폴백 체인 추출 (없으면 ready_no_mpp, 임의 기본값 금지), DB 저장 — §4-6
좌표계: 픽셀 기준, 북서쪽 원점 / BigTIFF: 4GB 초과 시 자동(명령은 항상 --bigtiff)
```
...
⑤ generate_kb_json() → Claude API로 kb JSON 초안 생성(엔진은 kb_generator 주입식 — API 키 비참조)
⑥ run_qc() → 타일 가독성·흰타일 비율·피라미드 무결성 검증(§4-4)
⑦ persist_result() → status·산출컬럼 DB UPDATE (★UPDATE-only, INSERT 아님 — 행 생성은 D30 배치파서)
```
**미니맵/썸네일 원칙**: 파이프라인이 미리 생성해 S3 저장, OpenSeadragon은 불러오기만.

> ✅ **현황(2026-06-12, 2단계)**: ①~⑦ 엔진 본체 구현 완료(extract_minimap/thumbnail 포함, pytest 362). 단 **이미지 연산 픽셀 경로는 로컬 미검증**(이 개발환경 pyvips 부재) → EC2 실파일 리허설 필요(§18 D35). 기존 'thumbnail openslide 동적 생성·/minimap 부재'는 엔진 구현으로 대체 경로 확보(D7 상태 갱신).
...
> **구현 상태(2026-06-12)**: 정찰 결과 기존 코드엔 이 계약이 **부재**였고 이번 세션에 처음 정의됨. **1단계**=계약(ConversionJob/Result)·상태머신(§4-5)·모듈 골격(변경 금지 계약 확정). **2단계**=변환 엔진 본체(7단계 함수·MPP 폴백 체인 §4-6·`persist_result` UPDATE-only·검수본 보존)·`HttpTriggerAdapter.parse`·S3 reader/writer 구현, pytest 362(회귀 0). 엔진 이식성 준수(boto3/DB/env 비참조, I/O 주입). **외부검증(Codex+Gemini) 대기·이미지 픽셀 경로 EC2 리허설 대기**(§18 D35).

### 4-4. QC 자동 검증 항목
| 항목 | 기준 | 실패 시 |
|------|------|---------|
| 타일 가독성 | 저·중·고 3레벨 타일 디코드 성공(로컬=HTTP 200 대응) | failed |
| 흰 타일 비율 | 샘플 타일 흰색 < 95% | failed |
| **피라미드 무결성** | 다중 페이지(≥2) + 페이지 치수 **단조감소**(레벨0이 최대) | failed |
| MPP 범위 | 0.1~1.0 μm/px | 경고, 계속 |
| 최소 해상도 | 5,000 px 이상 | failed |

> ★ **'레벨 수 일치' → '피라미드 무결성'으로 대체(2026-06-12)**: libvips 가변 레벨이라 '고정 N 일치'는 무의미. **MVP 줌 버그**(고배율→줌아웃 시 중간 배율 없이 확 건너뜀)의 진짜 원인은 '레벨 수'가 아니라 **입력 파일 피라미드의 단조감소 깨짐**이었음(실측: Motic L0→L1 841 폭락→L2 폭등, openslide level_count=1 → 코드 수정으로 안 고쳐지던 이유). 단조감소 검증이 libvips(통과)/GDAL(L0→L1 폭락 실패)를 직접 가른다. 134종 전부 COG 변환하므로 줌 버그는 구조적으로 해결(확인). overview_levels(전용 컬럼)엔 실제 생성 레벨 수 기록.
...
### 4-6. MPP 추출 폴백 체인 (2026-06-12 실측 확정)
**MPP는 파일에 동봉되나 위치가 형식마다 다르고 openslide 가 형식 미인식 시 None 을 준다** → **openslide 단독 의존 금지**. (실측: Motic 파일이 ImageDescription 에 `MPP=0.261438` 있는데 openslide 는 vendor=generic-tiff·mpp-x=None 반환.) `_extract_mpp`는 "여러 예상 위치를 순서대로 열어 처음 찾은 유효값 채택, 다 실패하면 None" 폴백 체인:

| 순서 | 갈래 | 검증 |
|------|------|------|
| ① | openslide `mpp-x`/`mpp-y` 프로퍼티 | 검증됨(형식 인식 시) |
| ② | ImageDescription `MPP=` 패턴 (`MPP\s*=\s*([\d.]+)`, Aperio·Motic) | ✅ **실측검증(Motic 0.261438)** |
| ③ | OME-TIFF `PhysicalSizeX`(+단위 환산) | ⚠ 미검증(실파일 전) |
| ④ | generic TIFF `XResolution`+`ResolutionUnit`→µm 환산 | ⚠ 미검증 |
| ⑤ | DICOM `PixelSpacing`(mm→µm) | ⚠ 미검증(v1.0 입력 아님) |
| ⑥ | 다 실패 → None → `ready_no_mpp` | — (임의 기본값 절대 금지) |

- **유효범위 검사 0.05~2.0 µm/px**(쓰레기 값 채택 방지). 단위환산(XResolution=단위당 픽셀 수 → 역수)·OME 단위(nm/µm/mm/cm) 정확.
- **검증 안 된 갈래(③④⑤)는 코드 docstring 에 '미검증' 명시** — 검증된 척하지 않음(§0-5 정신).
- **★ 뷰웍스 BIF MPP 위치 = 각 TIFF Directory XMP(TAG 700) 내 `<iScan ScanRes="...">`(§11; 예: Directory 1=400x ScanRes=0.1725)** — generic-TIFF `XResolution`(④)이 아닌 별도 갈래. **openslide(①)가 BIF를 인식하면 ①로 충당, None이면 ② 다음에 XMP `ScanRes` 직접 파싱 갈래를 삽입.** v1.0 실입력(뷰웍스 BIF)의 핵심 MPP 경로이나 **현장 실측 후 확정**(134종 도착 1장 검증, §18 신규 D).
- **임의 0.25 폴백 제거 완료**(server_render.py MPP 폴백·viewer.html `tojson`+null 가드, 부록1).
...
> **★ 검수본 보존 불변식 + 마커(2026-06-12, 2단계)**: 재처리(failed/ready_no_mpp→pending→재변환) 시 변환이 만든 kb '초안'이 검수자가 보완한 '최종본'을 덮으면 안 된다(§5-4). `persist_result`는 기존 kb 가 **검수 완료 마커(`reviewed`/`reviewed_at`/`is_reviewed`)** 를 가지면 초안 SET 에서 제외한다. ⚠ **전제: 검수 모달이 보완 저장 시 이 마커를 kb JSON 에 남겨야 함** — 현재 엔진만 마커를 *읽고*, 검수 모달 쪽 *쓰기*는 미구현 → 짝맞춤 필요(§18 신규 D37).
...
- **가입→인증 동선 — 발송 실패 시 VERIFY 진입 (v3.23, 커밋 `c075101`, 프론트 한정)**: register가 user(pending)+코드를 commit한 뒤 메일 발송 실패하면 `EMAIL_SEND_FAILED`(502, success:false) 반환 — 계정은 생성됐는데 프론트가 가입폼에 갇히던 문제. `_doSignup`이 `EMAIL_SEND_FAILED`를 '계정 생성됨+발송만 실패'로 구분해 **VERIFY 뷰로 보내고 재발송 유도**(SES 정상화 시 재발송으로 코드 수신). 그 외 에러(EMAIL_EXISTS·NOT_ON_ROSTER 등 계정 미생성)는 기존대로 가입폼 에러. **auth 백엔드 무수정**(EMAIL_SEND_FAILED 응답 그대로).
- **★ 커넥션 누수 스윕 (v3.23, 커밋 `9b811c8`·`d8f7a32`, D11 실질 해소)**: 'conn 획득 후 검증실패 early return이 `try` 밖 `release_db_conn`을 건너뛰던' 누수 패턴을 **portal(개별추가·업로드)·change_password·resend·register·verify·login 6경로에서 일소** — `release_db_conn`을 `finally` 단일 지점으로 이동(autocommit 복구 후), 정상·검증실패·예외 모든 경로에서 acquire 1==release 1. **★ 판정 로직·에러코드·검증 순서·세션 불변(release 구조만 정리)**. AST 재스캔으로 누수 시그니처 0 확인(server_render 3건은 inner try/finally 거짓양성). register/verify/login 수정은 CEO 승인 후 진행(판정 본문 무변경 diff 검증).
...
- **★ 과목을 '데이터 칼럼'→'업로드 컨텍스트'로 이동 (✅ 구현, v3.23, 커밋 `617b372`)**: 엑셀 매 행에 과목을 적던 것을 **"과목은 화면에서 선택, 엑셀은 3칸(이름|지위|이메일)"** 으로 전환 — '한 업로드 = 한 과목' 불변식(오타·다과목 혼입 방지). 명단 탭 상단 **'과목 선택'(`work-subject`, `_subscribed_subjects` 동적, 1개면 자동선택)** 컨트롤 → 업로드·양식 다운로드·개별추가가 그 선택 과목을 파라미터로 사용. **서버 재검증**: 업로드·양식·개별추가 모두 `_subscribed_subjects(g.institution_id)`에 없으면 행 처리 전 **403(SUBJECT_NOT_SUBSCRIBED)** — `?subject_code` 조작으로 비구독/타 기관 과목 등록 차단(§9 scope). 파서 3칸화(`_PORTAL_ROSTER_HEADER`=이름·지위·이메일), 좌석 재검사·skip-and-report·`_sync_member` 등 검증 로직 재사용(§0 데이터 모델 불변 — `users.subject_code`·roster 행은 여전히 과목별 채움, 다과목이면 과목별 따로 업로드). 행정직원은 양식 지위에서 제외(admin-only), 식별자=이메일 안내.
- **★ 포털 로그아웃 (✅ 구현, v3.23, 커밋 `3e02bd1`)**: 헤더에 '로그아웃' 버튼 추가 — 학생/교수와 동일 기존 메커니즘 재사용(`POST /api/auth/logout`, interceptor CSRF 자동, session_token=NULL+쿠키 삭제). 신규 경로 0.
...
**★ 뷰웍스 스캔 출력 형식 확정 = Roche BIF 기반 BigTIFF (2026-06-13, §4 입력)**: 뷰웍스(Vieworks LH210/LH510) 스캐너 출력 = **Roche BIF 기반 BigTIFF**. 특성:
- **MPP 위치**: 각 TIFF Directory의 XMP(TAG 700) 내 `<iScan ScanRes="...">`(Directory 1=400x ScanRes=0.1725 등). openslide 미인식 시 XMP 직접 파싱 갈래 필요(§4-6).
- **피라미드**: 2배 단조감소(libvips 변환에 적합). **원본 타일 1024×1024**(우리 표준 256×256으로 재타일링, §4-1).
- **DICOM 출력도 가능하나 MVP에서 DICOM 로딩 난이도로 입력 후보 제외** → **TIFF(BIF) 단일 입력**으로 고정.
- ⚠ **BIF 스티칭 리스크 추적**: Roche BIF는 타일 겹침(overlap)+소프트웨어 스티칭 구조라 openslide/libvips가 스티칭 불완전 시 줄무늬·이음새 아티팩트 보고 사례 있음(2015~18, Roche 자체 스캐너 기준). 단 뷰웍스는 Roche 장비 아닌 변형 BIF라 실제 문제 여부 미확정 → **윤정환 본부장에 '스티칭 시점(스캐너 단 vs 뷰어 단)' 문의 발송**. **134종 수요일 도착 후 1장으로 현장 실측**(줄무늬·MPP·피라미드·libvips 변환) 예정(§18 신규 D).
...
> ⚠ **포털·LMS 목업은 기능 검증 이전 스케치 (v3.23)** — 명단 과목 컨텍스트화·포털 로그아웃·교수/조교 수업관리 랜딩 분기·가입 동선(VERIFY 진입) 등은 **사양서(§9·§21) 기준이며 목업에 미반영**. 디자인 재제작(D34) 시 최신 사양 기준으로 재생성한다(목업 충돌 시 본 문서 우선).
...
| D7 | 미니맵/썸네일 파이프라인 | §4-2 "S3 사전 생성" vs (옛)동적 생성. **v3.24: 엔진에 `extract_minimap`/`extract_thumbnail` 구현됨**(최저 오버뷰→minimap.png, pyvips.thumbnail 400×300→thumbnail.jpg, writer로 S3) → '엔진 구현, 실파일 검증·라우트 연결 대기'로 상태 갱신(픽셀 경로 EC2 리허설 D35). **v3.21 비고**: LMS 3단계-B 수업상세·마이페이지 슬라이드 카드는 실썸네일 대신 아이콘 플레이스홀더 사용(게이트 무관 표시 원칙). | 🟠 엔진 구현/실파일 검증 대기 | Lead Developer |
...
| D11 | DB 커넥션 release 전수 | get_db_conn/release_db_conn 누수 전수 카운트 미완. **v3.22 비고**: 단계1에서 포털 명단 경로 RDS 왕복 5→감소(중복 admin 조회 제거), 단 한 요청 4 체크아웃 구조는 잔존 — 추가 최적화는 D14 Locust 시 측정 후 판단(D33). **v3.23 비고**: '검증실패 early return이 release 건너뜀' 누수 패턴을 AST 스윕으로 6경로(portal·change_password·resend·register·verify·login)에서 일소(release를 finally 단일지점으로, §8)→AST 재스캔 0. **실질 해소**. 잔여: server_render `_get_admin_user`·`_load_notices`·`admin_delete_slide`는 inner try/finally로 release 보장(거짓양성, 안전). | 🟡 추적(대부분 해소) | QA |
| D12 | 다중 과목 접근 (정책 확정 v3.2, Codex#3·Gemini#5) | ✅ 정책 확정: **이메일당 users 1계정, 과목 접근은 institution_rosters 행, users.email 전역 UNIQUE**(§6-2). register가 이메일 전역 검사로 중복 계정/pending 차단(앱 레이어). 잔여: ① DB 차원 email 전역 UNIQUE 제약(별도 마이그레이션) ② 단일 `users.subject_code` 게이트라 한 계정 다과목 동시 열람 미구현 — v1.5. (구분: 수업 다대다 등록은 `course_enrollments`로 v1.0 지원, 과목 구독과 별개 축 §21.) **v3.23 비고(Gemini)**: v1.5 다과목 구현 시 `_remove_member`가 활성 과목 회수 시 '남은 유효 과목으로 자동 전환'하는 대칭 로직 필요(현재 `_sync_member`엔 추가 동기화 있으나 제거 측 대칭 없음). 미구현 시 다과목 유저가 한 과목 제거로 접근 상실+`EMAIL_EXISTS` 재가입 불가=락아웃. **v1.0은 HST 단일이라 미발생**. | 🟠 v1.5 전(정책 확정) | Lead Developer |
...
| D31 | 학생 비밀번호 변경 API 부재 | 마이페이지(§21-8) 비번 폼이 표시용(토스트)+TODO 주석. 학생이 비밀번호 변경 불가 → 출시 전 필요. **✅ 해결(v3.23, `288f0bb`)**: `POST /api/auth/change-password`(본인 g.user_id·IDOR 차단·현재비번 검증·해시/세션 재사용, §21-8). mypage 폼 실연동. | ✅ 완료 | Lead Developer |
...
| D34 | 디자인 전면 재제작 | 현재 UI는 기능 검증용(군대 인트라넷 수준), 출시 불가. 의도된 디자인 시스템(LMS 목업 기준: navy/sky·Montserrat·Noto Sans KR·Tabler)으로 전 화면(포털·교수·학생·가입/로그인·뷰어) 재제작 필요. | 🔴 출시 전 필수 | 디자인 트랙 |
| D35 | 파이프라인 실가동 리허설 (이미지 픽셀 경로 EC2 검증) | **v3.24: 엔진 본체(2단계) 구현 완료**(pytest 362), 단 이 개발환경 pyvips 부재로 **convert_cog/minimap/thumbnail/QC 픽셀 경로는 로컬 미검증** → EC2 실파일(SVS 1~2장) end-to-end 리허설 필수(libvips 변환·S3 업로드·DB persist·타일 서빙). 134종 입고(2026-06 다음주) 전 소규모. 타일서버 systemd 미등록(INFRA_CHECKLIST F-1)·미니맵/썸네일 S3 사전생성(D7) 점검 동반. | 🔴 입고 전 | CEO/Lead |
| D36 | 타일서버 openslide→COG 전환 | 현행 타일서버는 openslide DeepZoom **동적** 타일링(§3 정정). 목표=사전 COG 변환본을 **range 읽기**로 서빙. **부하 방어의 실제 해결 지점**(동적 디코드 제거). 134종 COG 변환 정착 후 착수. | 🟠 변환 정착 후 | Lead Developer |
| D37 | 검수본 보존 마커 쓰기 측 미구현 | `persist_result`는 kb JSON 의 검수 마커(`reviewed`/`reviewed_at`/`is_reviewed`)를 **읽어** 초안 덮어쓰기를 막으나(§5-4), **검수 모달이 보완 저장 시 마커를 쓰는 경로는 미구현**. 짝맞춤 전엔 모든 기존 kb 가 '초안'으로 간주돼 재처리 시 덮일 수 있음. 검수 모달 구현과 함께. | 🟠 검수 모달 구현 시 | Lead Developer |
| D38 | 변환 워커 엔트리포인트 미구현 | pipeline/ 엔진·어댑터는 완성이나 **업로드→워커 트리거→`run()`→`persist_result`를 잇는 실행 루프**(EC2 워커 systemd/큐, §4-2 "EC2 워커 자동 트리거")는 미구현. 현재는 함수 단위만 호출 가능. D35 리허설·D30 배치적재와 맞물림. | 🔴 출시 전 | Lead Developer |
| D39 | slides.overview_levels 마이그레이션 | `db/slides_overview_levels_migration.sql`(멱등·트랜잭션, nullable INT) **작성 완료 + CEO EC2 psql 실행 완료**. `persist_result`의 `overview_levels` SET 대상 전용 컬럼(§4-1·§4-4). 미실행 시 persist 컬럼부재 실패였으나 실행으로 해소. | ✅ 완료(작성+실행) | CEO 실행 |
| D40 | 뷰웍스 BIF 스티칭 실파일 검증 | 뷰웍스 출력=Roche BIF 기반 BigTIFF(§11). Roche BIF는 타일 겹침+소프트웨어 스티칭 구조라 openslide/libvips 스티칭 불완전 시 줄무늬·이음새 아티팩트 사례 있음(2015~18, Roche 자체 스캐너). 뷰웍스는 변형 BIF라 실제 문제 여부 미확정. **134종 수요일 도착 후 1장으로 현장 실측**(줄무늬·MPP·피라미드·libvips 변환) — 134장 배치 변환 전 필수. 윤정환 본부장에 '스티칭 시점(스캐너 단 vs 뷰어 단)' 문의 발송, 회신 대기. D35 리허설과 동반. | 🔴 배치 전 필수 | CEO/Lead |
| D41 | 뷰웍스 BIF MPP 갈래(XMP TAG700 ScanRes) 추가 | v1.0 실입력(뷰웍스 BIF)의 MPP는 각 TIFF Directory XMP(TAG 700) `<iScan ScanRes="...">`에 기록(§4-6·§11). 현재 `_extract_mpp` 체인엔 전용 갈래 없음. **openslide(①)가 BIF를 인식하면 충당, None이면 ② 다음에 XMP `ScanRes` 직접 파싱 갈래 삽입** — D40 현장 실측(openslide BIF 인식 여부)으로 필요성 확정 후 구현. | 🟠 현장 실측 후 | Lead Developer |
...
> **★ 교수/조교 수업관리 랜딩 + 양방향 동선 (✅ 구현, v3.23, 커밋 `98faf4d`·`e7fb63b`)**: 교수가 학생과 동일하게 `/home`으로 랜딩해 수업 개설·관리 진입점이 안 보이던 문제 해결.
> - **position을 인증 응답에 전달(additive)**: `login`·`verify-email` 성공 응답 data에 `position` 추가(`_fetch_position`, DB 권위, 실패 시 None→/home 폴백). 기존 필드 순서·값 불변(§13-2), main transaction과 분리된 부가 조회 — 인증·세션 로직 무수정.
> - **랜딩 분기(`_postLoginDest`)**: ① admin-only(role='admin'·subject 없음)→`/portal`(겸직도 포털 우선) → ② position∈{교수,조교}→`/teacher/courses` → ③ 그 외(학생)→`next`(기본 /home).
> - **양방향 링크**: home top-bar '수업 관리'(→/teacher/courses)는 **`is_teacher`만**(학생 미노출, `/home` 핸들러가 position 재조회해 전달); teacher top-bar '슬라이드 열람'(→/home, 전체 탭=슬라이드 카탈로그).
> - **`/api/courses/mine` position 재검증(§8, Codex)**: `_course_position` DB 재조회로 **교수=소유(professor_user_id)/조교=위임(course_assistants)/그 외=fail-closed 빈목록** — 강등·박탈 즉시 반영(stale 소유·위임 메타 누수 차단). 조교 빈 상태 graceful 안내.
> - ★ **프론트 position은 '랜딩 힌트'일 뿐 — `/teacher` 접근 판정은 서버 `_course_position`(매 요청 DB)이 별도로 한다**(프론트 position 위조로 권한 우회 불가).
...
- **★ 비밀번호 변경 구현 (✅ v3.23, D31 해결, 커밋 `288f0bb`)**: `POST /api/auth/change-password`(`@login_required`, CSRF interceptor). **본인만(scope=`g.user_id`, body/경로 user_id 미참조 — IDOR 불가)**, 현재 비번 검증(`check_password_hash` 재사용, 불일치 시 `CURRENT_PASSWORD_INVALID` 403), 새 비번 정책(register와 동일 8자, `SAME_PASSWORD` 거부), 해싱(`generate_password_hash`)·세션 회전(`_issue_token_payload` 재사용 — 다른 기기 SESSION_REVOKED, 현재 기기 새 쿠키 유지). 신규 해시/세션 로직 0(기존 함수 재사용).
...
- **v3.23**: e2e 후 개선 묶음(1~6) 반영 — ① 명단 **과목 컨텍스트화**(`617b372`: 데이터 칼럼→업로드 컨텍스트, 3칸 엑셀·과목 선택·서버 재검증 403) ② **포털 로그아웃**(`3e02bd1`) ③ **가입 동선**(`c075101`: EMAIL_SEND_FAILED 시 VERIFY 진입, 프론트 한정) ④ **학생 비번변경**(`288f0bb`, D31 해결: change-password·IDOR 차단·현재비번 검증·해시/세션 재사용) ⑤ **교수/조교 수업관리 랜딩 + 양방향 동선**(`98faf4d`·`e7fb63b`: login/verify position additive·`_postLoginDest` 분기·home↔teacher 링크·`/api/courses/mine` `_course_position` 재검증 fail-closed). ★ **커넥션 누수 스윕**(`9b811c8`·`d8f7a32`: 6경로 'release 건너뜀' 일소, release를 finally로, 판정 불변, AST 재스캔 0 — D11 실질 해소). 외부검증(Codex+Gemini) 통과 **pytest 335**. **D31 ✅ 해결**, **D34(디자인 전면 재제작 🔴)·D35(파이프라인 점검·리허설 🔴) 신설**, D12 다과목 제거 대칭 정교화(Gemini)·D11 비고·§17 목업 낡음 주석. **organ(v3.x)·기존(PATH·slide_links·LMS 3A/3B·D1~D33) 무변경 additive.**
- **v3.24** (COG 파이프라인 §4 실측 정정 + 2단계 엔진 본체): 2026-06-12 SVS 실측·부록1~3·엔진 구현으로 §4 문서 전제를 실측 현실에 정정(§0-5 정신, 단 실측이 옛 문서보다 정확). ① **변환 라이브러리 libvips(pyvips) 확정**(같은 Motic SVS: libvips 9레벨 단조감소 완벽 26초 vs GDAL 3레벨 부실 37초), 검증 명령(`vips tiffsave --tile 256 --pyramid --compression jpeg --Q 85 --bigtiff`) §4-1. ② **타일엔진 서술 정정**(§3): 기존 "titiler+rasterio+COG"는 코드 불일치였고 **현행=openslide DeepZoom 동적**, 목표=COG range 읽기(전환은 D36=부하방어 실제 해결지점). ③ **피라미드 '7레벨 고정'→libvips 가변 레벨**(41675px=9레벨 관측), QC '레벨 수 일치'→**'피라미드 무결성(다중페이지+단조감소)'** 대체(§4-4) — **MVP 줌버그 원인 규명**: 레벨 수 아닌 입력 피라미드 단조감소 깨짐(Motic L0→L1 841 폭락), libvips COG 변환이 구조적 해결. overview_levels 전용 컬럼 기록(D39 마이그레이션 작성+실행 완료). ④ **MPP 폴백 체인** §4-6 신설(6갈래: openslide→ImageDescription `MPP=`✅실측→OME⚠→generic-TIFF XResolution⚠뷰웍스유력→DICOM⚠→None, 0.05~2.0µm 검사, 0.25 임의폴백 제거). ⑤ **pipeline/ 구현상태**(§4-3): 1단계=계약·상태머신·골격(코드에 계약 부재였음, 첫 정의), 2단계=엔진 본체(7단계·persist UPDATE-only·검수본 보존 마커)·**pytest 362**, 단 이미지 픽셀 경로 로컬 미검증(pyvips 부재)→EC2 리허설 필수. **D7(엔진 구현/실파일 대기)·D35(픽셀경로 EC2 검증 구체화) 갱신, D36(타일서버 COG 전환)·D37(검수본 마커 쓰기측 미구현)·D38(워커 엔트리포인트 미구현)·D39(overview_levels 마이그레이션 ✅) 신설.** **코드 무수정(문서만) — 엔진 구현은 별도 세션 산출물. 규칙·결정·부채 행 내용 보존, 실측으로 바뀐 사실만 정정.**
- **v3.25** (뷰웍스 BIF 입력 형식 확정 — COG 세션 additive): 2026-06-13 뷰웍스 스캔 출력 형식 조사로 v1.0 실입력을 확정·문서화(§11·§4-6). ① **뷰웍스(Vieworks LH210/LH510) 출력 = Roche BIF 기반 BigTIFF**(§11 신설): MPP는 각 TIFF Directory XMP(TAG 700) `<iScan ScanRes="...">`(예: Dir1=400x ScanRes=0.1725), 2배 단조감소 피라미드(libvips 적합), 원본 타일 1024×1024(256 재타일링), DICOM 출력 가능하나 로딩 난이도로 입력 제외→TIFF(BIF) 단일 입력. ② **§4-6 MPP 체인 정정**: 옛 ④ "뷰웍스 TIFF 유력" 추정을 BIF 사실로 교체 — 뷰웍스 MPP는 generic-TIFF `XResolution`(④)이 아닌 **XMP TAG700 `ScanRes` 별도 갈래**(openslide ①이 BIF 인식하면 충당, None이면 ② 다음 삽입). ③ **BIF 스티칭 리스크 추적**: Roche BIF 타일 겹침+소프트웨어 스티칭→openslide/libvips 불완전 시 줄무늬 아티팩트 사례(2015~18, Roche 자체 스캐너), 뷰웍스 변형 BIF라 미확정→윤정환 본부장 '스티칭 시점' 문의 발송·134종 수요일 도착 1장 현장 실측. **D40(BIF 스티칭 실파일 검증 🔴 배치 전)·D41(BIF MPP XMP ScanRes 갈래 🟠 현장 실측 후) 신설, D7/D35 비고 유지.** **코드 무수정(문서만). v3.24 COG/엔진 내용·규칙·결정·부채 행 무변경 additive — BIF 입력 사실만 추가.**
