...
| D20 | 현재-접근창 테스트 구독 생성 수단 부재(개발 편의) | 가을 런칭 기준이라 기관 등록 시작학기 선택지가 2026 가을부터·갱신은 다음 학기만 → 개발/QA 시 '지금 접근창 열린' 테스트 구독을 UI로 못 만들어 psql로 access_open_date 임시 조정 필요(e2e 사용). **2026-06 스모크용으로 TEST 기관 HST access_open_date를 2026-06-04로 임시 조정함 — 실데이터 전환 시 원복 검토.** | 🟡 추적(개발 편의) | Lead Developer/QA |
...
| D24 | 라이브 DB 잔재 정리 | 2026-06 스모크 발견. ① HS-PATH-004/005/006 — 옛 MVP 잔재, HS- 접두사로 §6-1 SA 단일채번 원칙 위반. ② 'TEST'(title_ko='x') 쓰레기 슬라이드 행. ③ SA-HST-0001 conversion_status=pending(변환 미완)·deploy_status=qc_pending. HST 134종 입고 시 함께 정돈. 삭제·재채번은 CEO 판단(콘텐츠 손실 주의). | 🔴 출시 전 | CEO |
...
*최종 업데이트: 2026-06-05 v3.9(2차): Codex 2차 재검증 반영 — Med#1 좌석 캐시가 pending user를 좌석 점유로 오산하던 §0 불일치 수정(_sync_member: user 조회에 status 포함, 메모리 좌석 증분·seat_full 게이트를 status='active'에만 적용 → active_seat_count 기준과 일치, pending 승격은 verify FOR UPDATE에 위임). Low#2 xlsx zip entry 상한 100→1000(시트·이미지 다수 정상 업무 파일 오탐 방지, 핵심 방어는 압축해제 총량+실측 크기+행/셀 상한). 전체 pytest 152 passed. D21·D22 추적 유지.*

*최종 업데이트: 2026-06-06 v3.10: P1 라이브 스모크(분기 A 통과) 기록 + D24(DB 잔재 정리) 신설 + D20 보강.*
