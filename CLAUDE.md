# CLAUDE.md — SlideAtlas 프로젝트 메모리 v2.5
...
**한 줄 정의**: 의과대학·치과대학·수의대·한의대·약대·간호대·보건전문대·전문대 내 관련학과들(임상병리학과 등)를 대상으로 한 디지털 병리/조직학 WSI(Whole Slide Image) 구독 SaaS 플랫폼.

**핵심 비즈니스 모델**: 오프라인 슬라이드 납품(보람바이오텍, 20년 의대 납품 이력) 네트워크를 디지털 구독 SaaS로 전환. 연 400만원 구독료, 장비 불필요(WinMedic 등 경쟁사 대비 차별점).

**핵심 포지셔닝**: "땡시(조직학 실습시험) 대비, 집에서 한국어로" — 교수가 주차별로 배치한 슬라이드를 학생이 집에서 WSI로 복습하며 실습시험 준비. Histology Guide(영어, 무료)와 달리 한국어 + 국가고시 연계 큐레이션 + 교수 수업 페이지 구조.

**경쟁 구도**: Histology Guide(무료 조직학 WSI 제공), WinMedic(스캐너+플랫폼 수직통합, 장비 수천만원) vs SlideAtlas(콘텐츠 구독 SaaS 연400만원, 장비 불필요) = 장비판매 vs Netflix
...
**품질 향상 3단계**:
1. 강화된 프롬프트로 AI 생성
2. 의대 대학원생 인턴이 검수·수정 (엑셀 기반 — 아래 QC 섹션 참조)
...
**기술 QC** (파이프라인 자동화) — 섹션 4-4 참조. 코드로 자동 처리.
...
### 6-2. Educational QC 인력 조달 루트

**루트**: 군대 고참 형님(참조은병원 신경외과 교수) → 서울대 기초조직학 교수 소개 → 대학원생 소개

**면담 목표 (서울대 교수님)**:
1. 143종 큐레이션 리스트 검토 — "수업에 못 쓸 것 같은 슬라이드" 피드백
2. Educational QC + knowledge_base 검수 작업 가능한 대학원생 1명 소개
3. (가능 시) 구매의향서(LOI) — 초창패 심사용, 부담 없는 수준으로

### 6-3. 대학원생 작업 범위 및 조건

**작업 내용**:
- 슬라이드 뷰어에서 각 슬라이드 열람 후 품질 판정 (흐림·염색불균일·아티팩트 등)
- AI 생성 knowledge_base JSON 검수 및 오류 수정
- 작업 결과물: 엑셀 파일 (slide_id별 QC 판정 + JSON 수정 내용)

**작업 분량 및 기간**:
- 143종 / 20일 기준 → 하루 7~8장
- 슬라이드 1장당 약 15~20분 (품질 체크 5분 + knowledge_base 검수 10~15분)
- 하루 2~3시간 투입으로 충분 → 대학원생 부업 수준

**조건**:
- 기간: 1개월
- 인건비: 150만원 (교수님 소개 관계 고려, 깔끔한 조건 유지)
- 추후 신규 슬라이드 추가 시 파트타임 지속 가능성 있음

### 6-4. 엑셀 기반 작업 방식 (v1.0)

관리자 JSON 편집 UI 개발 전, 엑셀로 운영:

```
컬럼: slide_id | title_ko | qc_status | qc_note | key_structures(수정) | exam_points(수정) | common_confusions(수정) | clinical_relevance(수정) | zoom_guide(수정)
```

- 대학원생이 엑셀 작성 → 보람님이 받아서 DB 반영
- 작업 후 불편사항 파악 → 이후 관리자 UI 개발 여부 결정

**품질 주의 항목** (educational QC 특히 엄격히):
...
CREATE TABLE users (
  ...
  -- 'student' | 'assistant' | 'professor' | 'institution_admin' | 'super_admin'
...
- knowledge_base JSON 편집 UI (대학원생 엑셀 검수 결과 반영용 — v1.0은 수동 반영)

### 기관 관리자 (/portal)
- 학생 명단: xlsx/csv 업로드 → DB 등록 (**지위 컬럼 포함**: 학생/조교/교수)
- 개별 학생 추가/삭제, 라이선스 현황
- TO 게이지: 전체 정원 대비 등록 인원 시각적 표시 (예: 185 / 200명)
...
| 공급사 | 상태 | 수량 | 비고 |
|--------|------|------|------|
| Happy Science (Mallen Zhang / Linda Li) | 계약 진행 중 | 조직학 143종 전체 커버 가능 | 최우선 파트너. $20/장, 로열티 없음. 중국 전역 소싱으로 missing 항목 보완 제안 |
| TCGA 오픈소스 | 사용 중 | 일부 | MVP용 |
| 3DHISTECH 샘플 | 사용 중 | 1종 (소장 H&E) | MVP용 |
| Vic Science (Joy Xu) | 견적 대기 | - | 물리 슬라이드 + WSI 견적 2종 요청 |
| Yulin (Jessy) | 라이선스 협의 중 | - | 윗선 논의 후 회신 예정. 예상보다 협조적 |
...
- AI 튜터 탈옥 질문 방어
- 교수 수업 페이지: 타 기관 교수가 다른 기관 수업 접근 차단
...
| 기간 | 목표 |
|------|------|
| ~7월 말 | 개발 완성 (JWT 인증, 기관 포털, 파이프라인, 동적 워터마킹, 서버사이드 캐시, 교수 수업 페이지) |
...
- SlideAtlas 차별점: ①큐레이션 콘텐츠 ②어디서든 접속 ③AI 튜터 동시 제공 ④한국어 땡시 대비 모드
...
**영업 네트워크**
- 경희대 의료원 신경외과 교수 (군대 고참 형님) — LOI + 서울대 기초조직학 교수 소개 루트
- 연세대 소아과 교수 (교회 형님) — 연대 내 타겟 교수 소개 루트
- 단국대 서민 교수 (기생충학 권위자, 아버지 지인) — LOI + 콘텐츠 파트너십
- 아버지 지방의대 납품 네트워크 — LOI + 실제 구독 타겟
- 구매의향서(LOI) 목표: 3~5개 (초창패 심사 매출 없는 약점 보완용)
...
- **9월 런칭 시**: 의대·치대 대학원생 파트타임 인턴 1명 (월 150만원, 1개월)
  - 역할: educational QC 검수 + knowledge_base JSON 수정 (엑셀 기반)
  - 채용 루트: 군대 고참 형님 → 서울대 기초조직학 교수 → 대학원생 소개
  - 이후 신규 슬라이드 추가 시 파트타임 지속 가능성 검토
...
- **COG 변환 배치**: SVS 1장당 5~15분, 143장 = 최대 35시간 → EC2 밤새 배치 실행
...
- Happy Science: Mallen Zhang (GM) / Linda Li / info@ihappysci.com / WhatsApp +86 188 3816 1683
...
**등록 방식**:
- 기관 관리자 엑셀 업로드 컬럼: `이메일 | 이름 | 지위(학생/조교/교수)`
- 가입 시 본인이 선택한 지위 + 엑셀 지위 대조 → 일치 시 인증 완료, 불일치 시 차단

**지위별 권한**:

| 지위 | 슬라이드 열람 | 퀴즈 | 수업 페이지 접근 | 수업 페이지 생성/편집 |
|------|-------------|------|----------------|-------------------|
| 학생 | ✅ | ✅ | (교수 페이지 열람만) | ❌ |
| 조교 | ✅ | ✅ | ✅ (교수 위임 시) | ✅ (위임받은 수업만) |
| 교수 | ✅ | ✅ | ✅ | ✅ (본인 수업) |
...
- **가입 방식**: 누구나 가입 시도 가능. 단, 기관 관리자가 사전 업로드한 명단과 이메일 + 지위가 모두 일치해야만 계정 활성화.
- **순서 중요**: 기관관리자 명단 업로드 → 학생 가입 순서여야 함. 계약 시 기관에 반드시 안내 필요.
- **소속 기관 및 지위**: 마이페이지에서 읽기 전용. 변경 필요 시 관리자 경유.
- **명단 수정 후 재가입 불필요**: 이미 입력한 정보로 자동 재대조.
- **로그인 성공 시**: 역할에 따라 다른 홈으로 자동 이동.

### 22-5. 마이페이지 구성

계정 활성화 완료 시 시스템이 자동 생성. 포함 항목:

- 프로필 정보 (이름 · 이메일 · 소속 · 지위 — 소속/지위는 읽기 전용)
...
**핵심 원칙**: TO(구독 인원) 내에서는 SlideAtlas가 관여하지 않는다.
...
슈퍼관리자가 기관관리자 이메일 등록
  → 포털 로그인 (/portal)
  → 학생 명단 관리 (xlsx 업로드 · 개별 추가/삭제) — 지위 컬럼 필수
...
교수 로그인
  ...
  → 수업 페이지 생성 (과목명, 학기)
  → 주차 추가 (예: 3주차 - 결합조직)
  → 슬라이드 추가 (143종 중 선택, drag-and-drop 배치)
  → 조교 추가 (수업별 개별 위임)
  → 학생에게 수업 페이지 링크 공유
...
학생/조교 접속
  → "수업" 탭에서 교수 수업 목록 확인
  → 주차별 슬라이드 열람
  → 기존 뷰어 + AI 튜터 + 퀴즈 탭 그대로 활용
```

**포지셔닝**: "땡시(실습시험) 대비, 집에서 한국어로"
- 교수가 주차별 슬라이드를 미리 배치 → 학생이 집에서 WSI로 예습/복습
- Histology Guide(영어, 무료) 대비 차별점: 한국어 + 국가고시 연계 + 교수 지정 커리큘럼

**v1.0 최소 구현 범위**:
- 교수: 수업 생성, 주차 추가, 슬라이드 배치, 조교 위임
- 학생/조교: 수업 목록 접속, 주차별 슬라이드 열람
- 기존 뷰어 + 퀴즈 탭 재활용 (신규 개발 최소화)
...
### 23-8. Aperio ImageScope 참조 기능 (차용 목록)
...
*최종 업데이트: 2026-05-29 v2.5*
*주요 변경: 송계용 교수 관련 내용 전체 제거. Educational QC 전략 재편 — 군대 고참 형님(참조은병원 신경외과 교수) → 서울대 기초조직학 교수 소개 → 대학원생 1명 채용 루트로 확정. 대학원생 조건: 1개월, 150만원, 엑셀 기반 작업. 섹션 6(Educational QC 운영 전략) 신규 추가. 섹션 번호 일부 조정(구 섹션 6→7, 7→8 등). 영업 네트워크 섹션 17로 통합 이동.*
